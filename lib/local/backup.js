...
import { listAllMeasurements, replaceAllMeasurements } from "./measurements";
import { uploadBackup, downloadBackup } from "@/lib/googleDrive";

export async function backupToDrive(userId) {
  const [tasks, dailyTaskTemplates, goals, waterLogs, sleepLogs, moodLogs, books, bodyMeasurements] =
    await Promise.all([
      listAllTasks(userId),
...
export async function restoreFromDrive(userId) {
  ...
  await Promise.all([
    ...
    replaceAllMeasurements(userId, backup.bodyMeasurements),
  ]);
  return true;
}
