...
import { setMeasurementField, listAllMeasurements } from "./measurements";
...
export async function seedSampleData(userId) {
  const [tasks, templates, goals, water, sleep, mood, books, measurements] = await Promise.all([
    listAllTasks(userId),
  ...
  const now = new Date();
  await setMeasurementField(userId, now.getFullYear(), now.getMonth(), "weight", 68);
}
