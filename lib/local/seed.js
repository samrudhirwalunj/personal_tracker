...
export async function seedSampleData(userId) {
  const [tasks, templates, goals, water, sleep, mood] = await Promise.all([
    listAllTasks(userId),
  ...
  if (tasks.length || templates.length || goals.length || water.length || sleep.length || mood.length) return;
