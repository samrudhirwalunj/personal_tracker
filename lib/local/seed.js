...
import { createTask, setTaskCompleted, listAllTasks } from "./tasks";
...
export async function seedSampleData(userId) {
  const [tasks, goals, water, sleep] = await Promise.all([
    listAllTasks(userId),
    listGoals(userId),
    listAllWaterLogs(userId),
    listAllSleepLogs(userId),
  ]);
  if (tasks.length || goals.length || water.length || sleep.length) return;

  const date = todayISO();

  const t1 = await createTask(userId, { title: "Morning stretch", taskDate: date, taskTime: "07:00" });
  await setTaskCompleted(userId, t1.id, true);
